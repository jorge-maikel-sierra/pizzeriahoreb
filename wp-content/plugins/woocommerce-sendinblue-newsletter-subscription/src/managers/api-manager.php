...
class ApiManager
{
    ...
    public function add_hooks()
    {
        ...
        add_action('woocommerce_order_status_on-hold', array( $this, 'on_order_status_on_hold' ), 11, 1 ); 
        ...
        add_action('woocommerce_new_customer_note_notification', array($this, 'on_new_customer_note'), 10, 1);
    ...
    public function on_order_status_changed($id, $status = 'pending', $new_status = 'on-hold')
    {
        ...
        if (strpos(SendinblueClient::CANCELLED_ORDER_STATUS, $new_status) !== false) {
            $this->trigger_admin_email_on_cancelled_order($order);
        }

        if (strpos(SendinblueClient::FAILED_ORDER_STATUS, $new_status) !== false) {
    ...
    private function wp_mail_template_customer_note($order, $customer_note, $email) {
        $email->is_enabled(false);
        $email->object = $order;
        $email_html = "";
        $email_plain = "";
        ...
        if ($email->get_content_type() == "text/plain") {
            $email_plain = $this->insert_customer_notes($email->get_content(), "The following note has been added to your order:", $customer_note);
        } else {
            $mod_html = $this->insert_customer_notes($email->get_content_html(), "The following note has been added to your order:", $customer_note);
            $email_html = apply_filters( 'woocommerce_mail_content', $email->style_inline( $mod_html ) );
        }
        
        return [$email_html, $email_plain];
    }

    private function insert_customer_notes($string, $keyword, $body) {
    ...
    public function on_new_customer_creation($customer_id, $new_customer_data, $password_generated) {
        ...
        if (isset($settings[SendinblueClient::IS_NEW_ACCOUNT_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_NEW_ACCOUNT_EMAIL_ENABLED]) {
            if ($settings[SendinblueClient::IS_NEW_ACCOUNT_TEMPLATE_ENABLED]) {
                ...
                $this->trigger_event_email_sib($new_customer_data['user_email'], $data, $settings[SendinblueClient::NEW_ACCOUNT_TEMPLATE_ID], self::EVENT_GROUP_SIB);
            } else {
                ...
                $this->trigger_event_email_woocommerce($new_customer_data['user_email'], $subject, $template, self::EVENT_GROUP_WOOCOMERCE);
            }
        }
    }

    public function on_new_customer_note($order)
    {
        $order_details = $this->if_email_enabled_get_order_details($order['order_id']);
        if (!$order_details)
        {
            return false;
        }
        ...
        if (isset($settings[SendinblueClient::IS_CUSTOMER_NOTE_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_CUSTOMER_NOTE_EMAIL_ENABLED]) {
            if ($settings[SendinblueClient::IS_CUSTOMER_NOTE_TEMPLATE_ENABLED]) {
                $this->trigger_event_email_sib($order_details['BILLING_EMAIL'], $order_details, $settings[SendinblueClient::CUSTOMER_NOTE_TEMPLATE_ID],  self::EVENT_GROUP_SIB);
            } else {
                $mailer = WC()->mailer();
                $email = $mailer->emails['WC_Email_Customer_Note'];
                $customer_note = $order['customer_note'];
                $order = $this->wc_get_order($order['order_id']);
                $template = $this->wp_mail_template_customer_note($order, $customer_note, $email);
                $subject = $email->get_subject();
                $this->trigger_event_email_woocommerce($order_details['BILLING_EMAIL'], $subject, $template, self::EVENT_GROUP_WOOCOMERCE);
            }
        }
    }
    ...
    public function on_order_status_completed($order_id)
    {
        ...
        if (isset($settings[SendinblueClient::IS_COMPLETED_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_COMPLETED_ORDER_EMAIL_ENABLED]) {
           if ($settings[SendinblueClient::IS_COMPLETED_ORDER_TEMPLATE_ENABLED]) {
                $this->trigger_event_email_sib($order_details['BILLING_EMAIL'], $order_details, $settings[SendinblueClient::COMPLETED_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
            } else {
                $mailer = WC()->mailer();
        ...
        if (isset($settings[SendinblueClient::IS_EMAIL_FEATURE_ENABLED]) && $settings[SendinblueClient::IS_EMAIL_FEATURE_ENABLED]) {
            ...
            if (isset($settings[SendinblueClient::IS_FAILED_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_FAILED_ORDER_EMAIL_ENABLED]) {
                if ($settings[SendinblueClient::IS_FAILED_ORDER_TEMPLATE_ENABLED]) {
                    ...
                    $this->trigger_event_email_sib($admin_email, $order_details, $settings[SendinblueClient::FAILED_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
                } else {
                    $template = $this->wp_mail_template_order($order, $email);
                    $subject = $email->get_subject();
                    $this->trigger_event_email_woocommerce($admin_email, $subject, $template, self::EVENT_GROUP_WOOCOMERCE);
                }
            }
        }
    }
        ...
        if (isset($settings[SendinblueClient::IS_EMAIL_FEATURE_ENABLED]) && $settings[SendinblueClient::IS_EMAIL_FEATURE_ENABLED]) {
            ...
            if (isset($settings[SendinblueClient::IS_CANCELLED_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_CANCELLED_ORDER_EMAIL_ENABLED]) {
                if ($settings[SendinblueClient::IS_CANCELLED_ORDER_TEMPLATE_ENABLED]) {
                    $order_details = $this->prepare_order_data($order);
                    $this->trigger_event_email_sib($admin_email, $order_details, $settings[SendinblueClient::CANCELLED_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
                } else {
                    $template = $this->wp_mail_template_order($order, $email);
                    $subject = $email->get_subject();
                    $this->trigger_event_email_woocommerce($admin_email, $subject, $template, self::EVENT_GROUP_WOOCOMERCE);
    ...
    public function on_order_status_on_hold($order_id)
    {
        ...
        if (isset($settings[SendinblueClient::IS_ON_HOLD_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_ON_HOLD_ORDER_EMAIL_ENABLED]) {
            if ($settings[SendinblueClient::IS_ON_HOLD_ORDER_TEMPLATE_ENABLED]) {
                $this->trigger_event_email_sib($order_details['BILLING_EMAIL'], $order_details, $settings[SendinblueClient::ON_HOLD_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
            } else {
                $mailer = WC()->mailer();
    ...
    public function on_order_status_refunded($order_id)
    {
        ...
        if (isset($settings[SendinblueClient::IS_REFUNDED_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_REFUNDED_ORDER_EMAIL_ENABLED]) {
            if ($settings[SendinblueClient::IS_REFUNDED_ORDER_TEMPLATE_ENABLED]) {
                $this->trigger_event_email_sib($order_details['BILLING_EMAIL'], $order_details, $settings[SendinblueClient::REFUNDED_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
            } else {
                $mailer = WC()->mailer();
    ...
    public function on_order_status_processing($order_id)
    {
        ...
        if (isset($settings[SendinblueClient::IS_PROCESSING_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_PROCESSING_ORDER_EMAIL_ENABLED]) {
            if ($settings[SendinblueClient::IS_PROCESSING_ORDER_TEMPLATE_ENABLED]) {
                $this->trigger_event_email_sib($order_details['BILLING_EMAIL'], $order_details, $settings[SendinblueClient::PROCESSING_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
            } else {
                $mailer = WC()->mailer();
        ...
        if (isset($settings[SendinblueClient::IS_EMAIL_FEATURE_ENABLED]) && $settings[SendinblueClient::IS_EMAIL_FEATURE_ENABLED]) {
            ...
            if (isset($settings[SendinblueClient::IS_NEW_ORDER_EMAIL_ENABLED]) && $settings[SendinblueClient::IS_NEW_ORDER_EMAIL_ENABLED]) {
                if ($settings[SendinblueClient::IS_NEW_ORDER_TEMPLATE_ENABLED]) {
                    $order_details = $this->prepare_order_data($order);
                    $this->trigger_event_email_sib($admin_email, $order_details, $settings[SendinblueClient::NEW_ORDER_TEMPLATE_ID], self::EVENT_GROUP_SIB);
                } else {
                    $template = $this->wp_mail_template_order($order, $email);
                    $subject = $email->get_subject();
                    $this->trigger_event_email_woocommerce($admin_email, $subject, $template, self::EVENT_GROUP_WOOCOMERCE);
    ...
    private function trigger_event_email_sib($to, $data, $template_id, $event_group)
    {
    ...
    private function trigger_event_email_woocommerce($to, $subject, $template, $event_group)
    {
