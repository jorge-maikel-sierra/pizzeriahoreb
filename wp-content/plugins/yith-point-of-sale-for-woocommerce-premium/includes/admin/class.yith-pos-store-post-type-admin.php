...
if ( ! class_exists( 'YITH_POS_Store_Post_Type_Admin' ) ) {
	...
	class YITH_POS_Store_Post_Type_Admin {
		...
		public function __construct() {
			...
			add_filter( 'manage_' . YITH_POS_Post_Types::$store . '_posts_columns', array( $this, 'manage_list_columns' ) );
			add_action( 'manage_' . YITH_POS_Post_Types::$store . '_posts_custom_column', array( $this, 'render_list_columns' ), 10, 2 );
			add_filter( 'default_hidden_columns', array( $this, 'default_hidden_columns' ), 10, 2 );
			add_filter( 'bulk_actions-edit-' . YITH_POS_Post_Types::$store, array( $this, 'manage_bulk_actions' ), 10, 1 );
		...
		public function render_filters( $post_type ) {
			...
			if ( YITH_POS_Post_Types::$store === $post_type ) {
				...
				echo "<option value=''>" . esc_html__( 'Filter by enabled status', 'yith-point-of-sale-for-woocommerce' ) . '</option>';
		...
		public function manage_row_actions( $actions, $post ) {
			if ( get_post_type( $post ) === YITH_POS_Post_Types::$store ) {
				...
				$actions['show-registers'] = "<a href='$show_registers_link'>" . esc_html__( 'Show Registers', 'yith-point-of-sale-for-woocommerce' ) . '</a>';
